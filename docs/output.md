...
This document describes the complete output structure and file formats generated by `python -m tfbpmodeling`.
...
Contains the complete bootstrap modeling results from Stage 1, where all features are used.
...
The single best-fit model trained on all data using only the significant coefficients from Stage 1.
...
Similar structure to `all_data_result_object/` but trained on the top-N data subset.
