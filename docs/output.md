...
This document describes the complete output structure and file formats generated by
`python -m tfbpmodeling`.

## Directory Structure

Results are saved in a subdirectory within your specified `--output_dir`:
...
- **Base directory**: Specified by `--output_dir` (default: `./tfbpmodeling_results`)
...
## Stage 1: Bootstrap Modeling Results (All Data)

### `all_data_result_object/`

Contains the complete bootstrap modeling results from Stage 1, where all features are
used.
...
- **Coefficient names**: Follow the model formula (e.g., interaction terms like
  "TF1:TF2")
...
- **`alpha_list`**: list of floats
  ...
  - Values: Optimal alpha (regularization parameter) selected by LassoCV in each
    iteration
...
**Interpretation:**
- These are the coefficients that survived Stage 1 filtering
- They will be used as the formula for Stage 2 (top-N modeling)
- Represents the first level of feature selection

## Stage 1: Best All Data Model

### `best_all_data_model.pkl`

The single best-fit model trained on all data using only the significant coefficients
from Stage 1.
...
## Stage 2: Top-N Modeling Results

### `topn_result_object/`

Similar structure to `all_data_result_object/` but trained on the top-N data
subset.
...
**Interpretation:**
- These coefficients survived both Stage 1 AND Stage 2 filtering
...
Only produced when `--stage3_lassocv_bootstrap` is passed. Contains significant
coefficients from the optional refit of surviving interactors together with their
independent main effects, using the same bootstrap LassoCV protocol as Stage 1 on all
data.
...
- Stage 3 - Lasso always uses `topn_significant_*.json` as its input; this output is
  independent and parallel to that test
...
- **`avg_r2_main_effect`**: Cross-validated R² when interaction is replaced by main
  effect
...
- **Positive delta_r2**: Main effect performs better than interaction (interaction may
  not be necessary)
...
- `stage3_lassocv_bootstrap_significant_{ci}.json` - Dictionary of significant
...
- `best_all_data_model.pkl` - Dictionary bundle containing fitted sklearn LassoCV model
  and metadata (use `joblib.load()`)
